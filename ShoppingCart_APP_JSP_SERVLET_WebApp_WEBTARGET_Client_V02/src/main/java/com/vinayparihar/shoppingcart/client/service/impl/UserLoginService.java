 ...
 * invocationBuilder.post(Entity.entity(login, MediaType.APPLICATION_JSON)); if
 * (response != null && (response.getStatus() == 200 || response.getStatus() ==
 * 201)) { user = response.readEntity(User.class);
 * 
 * if (user != null) { LOGGER.debug("Retriving User data. ");
 * 
 * LOGGER.debug("First Name : " + user.getFirstName());
 * LOGGER.debug("Last Name : " + user.getLastName()); LOGGER.debug("Email : " +
 * user.getEmail());
 * 
 * LOGGER.debug("Phone : " + user.getPhone()); LOGGER.debug("Password : " +
 * user.getPassword()); LOGGER.debug("Address Line 1 : " +
 * user.getAddress().getLineOne()); LOGGER.debug("Address Line 2 : " +
 * user.getAddress().getLineTwo()); LOGGER.debug("City : " +
 * user.getAddress().getCity()); LOGGER.debug("State : " +
 * user.getAddress().getState()); LOGGER.debug("Zipcode : " +
 * user.getAddress().getZipcode()); LOGGER.debug("Country : " +
 * user.getAddress().getCountry());
 * 
 * LOGGER.debug("User successfully Loggedin. "); return user; } } return null; }
 * }
 */
